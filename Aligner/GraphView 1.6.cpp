// $Id: GraphView.cpp,v 1.5 2011/10/11 08:58:41 riar Exp $
//
...
BOOL CGraphView::SetXYData( int serie, double xData, double yData )
{
  if( ( serie >= MIN_NO_OF_SERIES ) && ( serie <= MAX_NO_OF_SERIES ) )
  {
    ...
    m_GraphDataX[serie] = xData;
    m_GraphDataY[serie] = yData;
    m_ValidGraphDataToBeUpdated[serie] = TRUE;
...
BOOL CGraphView::SetSerieLabel( int serie, CString serieLabel )
{
  BOOL result = TRUE;

  if( ( serie >= MIN_NO_OF_SERIES ) && ( serie <= MAX_NO_OF_SERIES ) )
  {
    m_SerieLabels[serie] = serieLabel;
    try
    {
      int i;
      m_pGroup->GetSeriesLabels()->RemoveAll();
      ...
      {
        m_pGroup->GetSeriesLabels()->Add( m_SerieLabels[i].AllocSysString() );
      }
      ...
      if( IsAxisY2Enabled() == TRUE )
      {
        ...
        {
          m_pGroupY2->GetSeriesLabels()->Add( m_SerieLabels[i].AllocSysString() );
        }
      }
    }
    catch ( _com_error e )
...
BOOL CGraphView::ShowPolarGraphWithText( PolarGraphInParam* pInParam )
{
  ...
  try
  {
    ...
    for( i=1; i<=m_NoOfSeries; i++ )
    {
      ...
      str.Format( _T("%2.2f"), m_GraphDataY[i] );
      ...
      {
        str.Format( _T("%2.1f"), GetRange0ToPlusMinus180( m_GraphDataX[i] ) );
      }
      else
      {
        str.Format( _T("%2.1f"), m_GraphDataX[i] );
      }
  ...
  try
  {
    if( pInParam->writeTiltData == FALSE )
    {
      ...
      for( i=1; i<=m_NoOfSeries; i++ )
      {
        if( (pInParam->mode == Stab) || (pInParam->mode == StabDiff) || (pInParam->mode == RPath) )
        {
					SetSerieLabel( i, GetUnitDescription( g_AlignerData.ObjNo[i] ).Left( MAX_NO_OF_CHAR_IN_LEGEND_LABEL ) );
        }
        else
        {
					CString serieLabel;
					if( pInParam->commonFlatTest == TRUE )
					{
						...
						AfxFormatString1( serieLabel, IDS_SN_XXX, GetSensorSerialNumber( g_AlignerData.ObjNo[j] ) );
					}
					else
					{
						serieLabel = (GetUnitDescription( g_AlignerData.ObjNo[j] )).Left( MAX_NO_OF_CHAR_IN_LEGEND_LABEL );
					}
...
//        TRACE("\n MAX_NO_OF_SAMPLES_IN_GRAPH");
      ...
      else
      {
        if( m_NoOfSeries > 0 )
        {
          ...
          m_pData->PutX( ( long )serie, pointCount + 1 , m_GraphDataX[serie] );
          m_pData->PutY( ( long )serie, pointCount + 1 , m_GraphDataY[serie] );
        ...
        if( ( IsAxisY2Enabled() == TRUE ) && ( m_NoOfSeriesY2 > 0 ) )
        {
          ...
          m_pDataY2->PutX( ( long )serie, pointCountY2 + 1 , m_GraphDataX[m_NoOfSeries + serie] );
          m_pDataY2->PutY( ( long )serie, pointCountY2 + 1 , m_GraphDataY[m_NoOfSeries + serie] );
...
//        TRACE("\n MAX_NO_OF_SAMPLES_IN_GRAPH");
      ...
      else
      {
        if( m_NoOfSeries > 0 )
        {
          ...
          for( serie=1; serie<=m_NoOfSeries; serie++ )
          {
            if( m_ValidGraphDataToBeUpdated[serie] == TRUE )
            {
              pointCount = m_pData->GetNumPoints( serie );
              m_pData->PutNumPoints( serie, pointCount + 1 );
              m_pData->PutX( ( long )serie, pointCount + 1 , m_GraphDataX[serie] );
              m_pData->PutY( ( long )serie, pointCount + 1 , m_GraphDataY[serie] );
              m_ValidGraphDataToBeUpdated[serie] = FALSE;
            }
          }
          m_pData->PutIsBatched( FALSE );
        }
        ...
        if( ( IsAxisY2Enabled() == TRUE) && ( m_NoOfSeriesY2 > 0 ) )
        {
          ...
          for( serie=1; serie<=m_NoOfSeriesY2; serie++ )
          {
            if( m_ValidGraphDataToBeUpdated[m_NoOfSeries + serie] == TRUE )
            {
              pointCountY2 = m_pDataY2->GetNumPoints( serie );
              m_pDataY2->PutNumPoints( serie, pointCountY2 + 1 );
              m_pDataY2->PutX( ( long )serie, pointCountY2 + 1 , m_GraphDataX[m_NoOfSeries + serie] );
              m_pDataY2->PutY( ( long )serie, pointCountY2 + 1 , m_GraphDataY[m_NoOfSeries + serie] );
              m_ValidGraphDataToBeUpdated[m_NoOfSeries + serie] = FALSE;
            }
          }
          m_pDataY2->PutIsBatched( FALSE );
        }
...
void CGraphView::OnBnClickedLiveGraphQuit( void )
{
  ...
  if( (theApp.IsAligner202Enabled() == FALSE) && (m_exitWithoutSave == FALSE) )
  {
...
BOOL CGraphView::UpdateSourceBarData( void )
{
  ...
  xPos = (double)(m_ClusterWidthForOneBar + (double)(( 2 * m_NoOfSeries - 1 ) * m_ClusterWidthForOneBar) / 2.0) / 100.0;
	...
	{
		gunRoll = GetRoll( g_AlignerData.ObjNo[i], m_currentValues );
...
BOOL CGraphView::UpdateLiveDataGraph( void )
{
  ...
  BOOL updateGraph;
  int i, serieCounter;
  double rollValue, pitchValue, temperature;
  BOOL ready = FALSE;
  m_LiveDataCounter++;
  rollValue = pitchValue = temperature = 0;
  
  updateGraph = TimeToUpdate( m_LiveDataCounter );
  ...
  double xPos;
    ...
    if( theApp.IsAligner202Enabled() == TRUE )
    {
      rollValue = MRADIANS_TO_DEGREES( GetUncalibratedRoll( g_AlignerData.ObjNo[i], m_currentValues ) );
      AdjustSyncroValueToRangeA202( &rollValue, GetGearing( g_AlignerData.ObjNo[i], m_pSource ) );
      pitchValue = rollValue;
      temperature = 20.0;
    }
    else
    {
      rollValue = - g_AlignerData.SignDef * GetRoll( g_AlignerData.ObjNo[i], m_currentValues );
		  pitchValue = - g_AlignerData.SignDef *GetPitch( g_AlignerData.ObjNo[i], m_currentValues );
		  temperature = GetTemperature( g_AlignerData.ObjNo[i], m_currentValues  );
    }
    ...
    if( rollValue > m_MaxLiveGraphRollValue[i] )
    {
      m_MaxLiveGraphRollValue[i] = rollValue;
    }
    if( rollValue < m_MinLiveGraphRollValue[i] )
    {
      m_MinLiveGraphRollValue[i] = rollValue;
    }

    if( pitchValue > m_MaxLiveGraphPitchValue[i] )
    {
      m_MaxLiveGraphPitchValue[i] = pitchValue;
    }
    if( pitchValue < m_MinLiveGraphPitchValue[i] )
    {
      m_MinLiveGraphPitchValue[i] = pitchValue;
    }
    ...
    if( updateGraph == TRUE )
    {
			...
			if( fabs( (m_PreviousLiveGraphRollValue[i] - m_MaxLiveGraphRollValue[i]) ) > 
					fabs( (m_PreviousLiveGraphRollValue[i] - m_MinLiveGraphRollValue[i]) ) )
			{
				rollValue = m_MaxLiveGraphRollValue[i];
			}
			else
			{
				rollValue = m_MinLiveGraphRollValue[i];
			}

			if( fabs( (m_PreviousLiveGraphPitchValue[i] - m_MaxLiveGraphPitchValue[i]) ) > 
				fabs( (m_PreviousLiveGraphPitchValue[i] - m_MinLiveGraphPitchValue[i]) ) )
			{
				pitchValue = m_MaxLiveGraphPitchValue[i];
			}
			else
			{
				pitchValue = m_MinLiveGraphPitchValue[i];
			}

      SetXYData( serieCounter++, xPos, rollValue );

      if( theApp.IsAligner202Enabled() == FALSE )
      {
        SetXYData( serieCounter++, xPos, pitchValue );
      }
      ...
      m_MinLiveGraphPitchValue[i] = MIN_LIVE_GRAPH_RESET_VALUE;
			m_PreviousLiveGraphRollValue[i] = rollValue;
			m_PreviousLiveGraphPitchValue[i] = pitchValue;
...
BOOL CGraphView::ShowBarGraphWithText( BarGraphInParam* pInParam )
{
  ...
  try
  {
		...
		for( i=1; i<=g_AlignerData.NoOfCorr; i++ )
    {
			...
			SetSerieLabel( j++, GetUnitDescription( g_AlignerData.ObjNo[i] ).Left( MAX_NO_OF_CHAR_IN_LEGEND_LABEL ) );
    }
  }
  catch ( _com_error e )
  {
...
BOOL CGraphView::WriteLiveDataFileHeader( LiveDataGraphInParam* pInParam )
{
  ...
  CloseLiveDataFile();
	...
	if( m_LiveDataFile.Open( pInParam->FileName, CFile::modeWrite|CFile::modeCreate, NULL ) ) 
	{
		...
		tmpStr.Format( _T("%d"), 10 + g_AlignerData.NoOfCorr );
		...
		strArray[1+g_AlignerData.NoOfCorr] = _T(" ");
    strArray[2+g_AlignerData.NoOfCorr] = _T("Spec.");

		for( int i=1; i<=g_AlignerData.NoOfCorr; i++ )
		{
			//tmpStr.Format( _T("%d"), g_AlignerData.ObjNo[i] );
			tmpStr = GetChannelName( g_AlignerData.ObjNo[i] );
      strArray[i] = _T("Chan. ") + tmpStr + _T(": ") + GetUnitDescription( g_AlignerData.ObjNo[i] );
			str = _T(" Chan. ") + tmpStr;
			RAdjText( &str, 9 );
      strArray[2+g_AlignerData.NoOfCorr] += str;
		}

		for( int i=1; i<=2+g_AlignerData.NoOfCorr; i++ )
		{
...
BOOL CGraphView::WriteLiveDataFileHeaderA202( LiveDataGraphInParam* pInParam )
{
	...
	if( m_LiveDataFile.Open( pInParam->FileName, CFile::modeWrite|CFile::modeCreate, NULL ) ) 
	{
		...
		str = _T("Object: ") + g_AlignerData.Ship + END_OF_TEXT_FILE_LINE;
		m_LiveDataFile.Write( str, str.GetLength() );
		...
		for( int i=1; i<=g_AlignerData.NoOfCorr; i++ )
		{
			channelName = GetChannelName( g_AlignerData.ObjNo[i] );
            tmpStr = channelName;
            ...
            {
                gearing = GetGearing( g_AlignerData.ObjNo[i], m_pSource );
                tmpStr.Format( _T("%.1f"), gearing );
            }
            RAdjText( &tmpStr, columnWidth );
            strArray[1] += tmpStr + _T(" ");

            tmpStr = GetUnitDescription( g_AlignerData.ObjNo[i] );
            RAdjText( &tmpStr, columnWidth );
...
BOOL CGraphView::ShowLiveDataGraphWithText( LiveDataGraphInParam* pInParam )
{
  ...
  try
  {
    if( pInParam->initGraph == TRUE )
    {
      ...
      CString unitDescription;

      if( theApp.IsAligner202Enabled() == TRUE )
      {
        for( i=1; i<=m_NoOfSeries; i++ )
        {
          unitDescription = (GetUnitDescription( g_AlignerData.ObjNo[i] )).Left( MAX_NO_OF_CHAR_IN_LEGEND_LABEL );
          AfxFormatString1( str, IDS_UNITDESCRIPTION_GRAPH_Y1_LABEL, unitDescription );
          SetSerieLabel( i, str );
        }
        for( ; i<=m_NoOfSeries + m_NoOfSeriesY2; i++ )
        {
          unitDescription = (GetUnitDescription( g_AlignerData.ObjNo[i] )).Left( MAX_NO_OF_CHAR_IN_LEGEND_LABEL );
          AfxFormatString1( str, IDS_UNITDESCRIPTION_GRAPH_Y2_LABEL, unitDescription );
          SetSerieLabel( i, str );
        }
      }
      else
      {
        ...
        for( i=1; i<=m_NoOfSeries; i++ )
        {
          unitDescription = (GetUnitDescription( g_AlignerData.ObjNo[j] )).Left( MAX_NO_OF_CHAR_IN_LEGEND_LABEL );
      ...
      else
      {
        ...
        for( i=1; i<=g_AlignerData.NoOfCorr; i++ )
        {
          ...
          {
            j++;
				    str = GetUnitDescription( g_AlignerData.ObjNo[i] ).Left( MAX_NO_OF_CHAR_IN_LEGEND_LABEL );
            ...
            str.Format( _T("%2.1f"), GetTemperature( g_AlignerData.ObjNo[i], m_currentValues ) );
      ...
      if( theApp.IsAligner202Enabled() == FALSE )
      {
        ...
        for( i=1; i<=g_AlignerData.NoOfCorr; i++ )
        {
          if( IsSensor( g_AlignerData.ObjNo[i] ) == TRUE )
          {
            sensorNo++;
            //str.Format( _T("%2.1f"), m_PreviousLiveGraphTemperature[i] );
            double temp = GetTemperature( g_AlignerData.ObjNo[i], m_currentValues );
