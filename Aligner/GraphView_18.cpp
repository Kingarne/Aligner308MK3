// $Id: GraphView.cpp,v 1.18 2013/04/02 08:28:15 riar Exp $
//
...
//     }
    ...
    long textTempUnitLabelXPos = verticalLine1XPos + 3*MAX_WIDTH_OF_CHARACTER;
        ...
        if( pInParam->writeTiltData == FALSE )
        {
            ...
            //for( i=1; i<=m_NoOfSeries; i++ )
            {
                if( (pInParam->mode == Stab) || (pInParam->mode == StabDiff) || (pInParam->mode == RPath) )
                {
                    ...
                    AddGraph(i++, GetUnitDescription(iter->second.id).Left( MAX_NO_OF_CHAR_IN_LEGEND_LABEL ), iter->second.id);
                }
                else
                {
	                CString serieLabel;
            ...
            AddChartLabel("temperatureUnitlabel", str, textTempUnitLabelXPos, tempLabelYPos + j++ * ROW_HEIGHT);
            ...
            for( i=FIRST_ARRAY_INDEX_SENSOR; i<=LAST_ARRAY_INDEX_SENSOR; i++ )
            {
                ...
                AddChartLabel(tmpStr, str, textTempUnitLabelXPos, tempLabelYPos + j++ * ROW_HEIGHT );               
            }      
        }
...
BOOL CGraphView::UpdateSourceBarData( void )
{
    ...
    {     	
        gunRoll = GetRoll( m_graphData[gr].id, m_currentValues );
...
BOOL CGraphView::UpdateLiveDataGraph( void )
{
   // stw.Start();           
	...
	{
        if( 1)//theApp.IsAligner202Enabled() == TRUE )
        {
    ...
    {        
	    ...
	    if( m_StoreLiveDataToFile == TRUE )
	    {
            ...
            else
            {
                ...
                if( IsSyncro( m_graphData[gr].id ) == TRUE )
                {
                    tmpStr = _T("---");
                }
                else
...
BOOL CGraphView::ShowBarGraphWithText( BarGraphInParam* pInParam )
{
    ...
    try
    {
        ...
        for(iter=g_AlignerData.liveGraphMap.begin(); iter!=g_AlignerData.liveGraphMap.end(); iter++)
        {
		    AddGraph(j++, GetUnitDescription(iter->second.id).Left( MAX_NO_OF_CHAR_IN_LEGEND_LABEL ), iter->second.id);
...
BOOL CGraphView::WriteLiveDataFileHeaderA202( LiveDataGraphInParam* pInParam )
{
	...
	if( m_LiveDataFile.Open( pInParam->FileName, CFile::modeWrite|CFile::modeCreate, NULL ) ) 
	{
	    ...
	    for(int graph = 1; graph <=m_totalNoSeries ; graph++)
        {
            ...
            tmpStr = GetUnitDescription( m_graphData[graph].id );
            RAdjText( &tmpStr, columnWidth );
        ...
        strArray[0] = END_OF_TEXT_FILE_LINE;
    ...
    {        
        ...
        {    
            if( IsSensor( iter->second.id ))
            {
                ...
                AddChartLabel(tmpStr, str, textTempUnitLabelXPos, tempLabelYPos + hasTempr * ROW_HEIGHT);                                
            }
        }
        if( hasTempr > 0 )
        {
            ...
            str.LoadString( IDS_GRAD_CELCIUS_UNIT );
            AddChartLabel("temperatureUnitlabel", str, textTempUnitLabelXPos, tempLabelYPos);
...
CString CGraphView::GetGraphText(LiveGraphInfo& info, int dataType)
{
    ...
    CString unitDescription = (GetUnitDescription( info.id ));

    if(theApp.IsAligner202Enabled())
    {
      
        str.Format("%s: %s, %s%s, %s",GetAxisText(info, dataType), info.name, info.type, info.dig ? " - " + m_dataTypeText[dataType] : "", unitDescription );                        
    }
    else
    {        
        str.Format("%s: %s, %s",m_dataTypeText[dataType], info.name, unitDescription );                            
    }
