...
CString SecretRegistry::m_regPath = SECREGKEY_PATH;    
CString SecretRegistry::m_strAppName = "";
...
bool SecretRegistry::KeyExists(const CString& strKeyName)
{
    const int BUF_SIZE = 1024;
    DWORD nBufLen = BUF_SIZE;
    char aczSecretRegistryString[BUF_SIZE];
    ...
    HKEY hKey;
    int nResult;
    if((nResult = ::RegOpenKeyEx(HKEY_LOCAL_MACHINE, strKeyPath, 0, KEY_READ, &hKey)) == ERROR_SUCCESS)
    {
        nResult = ::RegQueryValueEx(hKey, strKey, NULL, NULL, (LPBYTE)aczSecretRegistryString, &nBufLen);
    }
...
CString SecretRegistry::GetStringValue(const CString& strKeyName, const CString& strDefaultValue)
{
  HKEY hKey;
  DWORD nDisposition;
  const int BUF_SIZE = 1024;
  ...
  //The RegCreateKeyEx function creates the specified SecretRegistry key.
  //If the key already exists, the function opens it.
  if ((nResult = ::RegCreateKeyEx(HKEY_LOCAL_MACHINE,
                                  strKeyPath, 
                                  0, 
                                  "", 
                                  0,
  ...
  //Try to read the value
                                        ...
                                        NULL, 
                                        NULL,
                                        (LPBYTE)aczSecretRegistryString, 
                                        &nBufLen)) != ERROR_SUCCESS)
  {

    //Write default value to key
    strcpy(aczSecretRegistryString, strDefaultValue);
    nBufLen = strlen(aczSecretRegistryString) + 1;

    ::RegSetValueEx(hKey,                       // handle to key
                    strKey,                     // value name
                    0,                          // reserved
                    REG_SZ,                     // value type
                    (LPBYTE)aczSecretRegistryString,  // value data
  ...
  //Everything went OK, return the value from SecretRegistry
  else
  {

    //Read value, return it
    strRetString = CString(aczSecretRegistryString);
  }

  //Close the SecretRegistry
  ::RegCloseKey(hKey);
...
//exist, it is created.
int SecretRegistry::SetStringValue(const CString& strKeyName, const CString& strValue)
{
  HKEY hKey;
  DWORD nDisposition;
  const int BUF_SIZE = 1024;
  ...
  //Write string to key
  strcpy(aczSecretRegistryString, strValue);
  nBufLen = strlen(aczSecretRegistryString) + 1;

  //The RegCreateKeyEx function creates the specified SecretRegistry key.
  //If the key already exists, the function opens it.
  if ((nRetVal = ::RegCreateKeyEx(HKEY_LOCAL_MACHINE,
                                  strKeyPath, 
                                  0, 
                                  "", 
                                  0,
...
//exist, it is created and the default value is stored in 
//the key.
int SecretRegistry::GetIntegerValue(const CString& strKeyName, int nDefaultValue)
{
  HKEY hKey;
  DWORD nDisposition;
  DWORD nBufLen = 4;
  int nSecretRegistryValue;
  int nResult;
  int nRetVal;
  CString strKey = strKeyName;
  ...
  //Try to read the value
                                        ...
                                        NULL, 
                                        NULL,
                                        (LPBYTE)&nSecretRegistryValue, 
                                        &nBufLen)) != ERROR_SUCCESS)
  {

    //Write default value to key
    nSecretRegistryValue = nDefaultValue;

    ::RegSetValueEx(hKey,                       // handle to key
                    strKey,                     // value name
                    0,                          // reserved
                    REG_DWORD,                  // value type
                    (LPBYTE)&nSecretRegistryValue,    // value data
                    nBufLen);                   // size of value data                    
...
int SecretRegistry::SetIntegerValue(const CString& strKeyName, int nValue)
{
  HKEY hKey;
  DWORD nDisposition;
  DWORD nBufLen = 4;
...
CString SecretRegistry::GetKeyPath(CString& strKeyName)
{
  ...
  if (strSubkey != "")
  {
    strKeyPath = SECREGKEY_PATH + m_strAppName + "\\" + strSubkey;
  }
  else
  {
    strKeyPath = SECREGKEY_PATH + m_strAppName + "\\";
  }
  
  return(strKeyPath);
}

std::vector<CString> SecretRegistry::GetMultiStringValue(const CString& strKeyName, std::vector<CString> &defaultValue)
{
  HKEY hKey;
  DWORD nDisposition;
  const int BUF_SIZE = 1024;
  ...
  //Try to read the value
                                        ...
                                        &nBufLen)) != ERROR_SUCCESS)
  {
    // Entry not in SecretRegistry, create with default values.
...
int SecretRegistry::SetMultiStringValue(const CString& strKeyName, std::vector<CString>& value)
{
  HKEY hKey;
  DWORD nDisposition;
  int nRetVal;
...
//Set a value in the SecretRegistry. If the key doesn´t 
//exist, it is created.
int SecretRegistry::SetBinaryValue(const CString& strKeyName, std::vector<BYTE>& value)
{
  HKEY hKey;
  DWORD nDisposition;
  
  DWORD size = value.size();
  BYTE* SecretRegistryVector = new BYTE[size];
  int nRetVal;
  CString strKey = strKeyName;
  ...
  //Try to write the value
                      ...
                      (LPBYTE)SecretRegistryVector,  // value data
...
//exist, it is created and the default value is stored in 
//the key.
std::vector<BYTE> SecretRegistry::GetBinaryValue(const CString& strKeyName, std::vector<BYTE>& defaultValue)
{
  HKEY hKey;
  DWORD nDisposition;
  
  DWORD size = 256;//value.size();
  BYTE* pSecretRegistryVector = new BYTE[size];
  int nResult;
  ...
  else
  {
    ...
    {
      retVector.push_back(pSecretRegistryVector[i]); 
    }    
  }
