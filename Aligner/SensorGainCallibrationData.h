// $Id: SensorGainCallibrationData.h,v 1.1 2009/10/14 08:08:14 riar Exp $
//
...
[
	db_command(L"SELECT dauSerialNumber, Id, operator, pitch, pitchTime, pitchTemperature, roll, rollTemperature, rollTime, serialNumber FROM SensorGainCallibrationData")
]
class SensorGainCallibrationData
{
public:
	...
	[ db_column(4, status=m_dwpitchStatus, length=m_dwpitchLength) ] double m_pitch ;	// Pitch gain value
	[ db_column(5, status=m_dwpitchTimeStatus, length=m_dwpitchTimeLength) ] double m_pitchTime ;	// Time of pitch callibration measurment start.
	...
	[ db_column(7, status=m_dwrollStatus, length=m_dwrollLength) ] double m_roll ;	// Roll gain value
...
[
  db_command { SELECT dauSerialNumber, Id, operator, pitch, pitchTime, pitchTemperature, roll, rollTemperature, rollTime, serialNumber FROM SensorGainCallibrationData WHERE serialNumber = ? }
]
class SensorGainCallibrationDataFromSerialNumber
{
public:
	[ db_column(1, status=m_dwdauSerialNumberStatus, length=m_dwdauSerialNumberLength) ] TCHAR m_dauSerialNumber [4] ;	// Serial number of the Data Accusition Unit (DAU) used for callibration.
	[ db_column(2, status=m_dwIdStatus, length=m_dwIdLength) ] LONG m_Id ;	// Primary index
